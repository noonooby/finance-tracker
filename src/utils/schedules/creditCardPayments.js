...
export async function processOverdueCreditCardPayments(creditCards, bankAccounts = [], cashInHand = 0) {
  try {
    ...
    console.log('🔄 Processing overdue credit card payments...');
    ...
    if (overdueCards.length === 0) {
      console.log('✅ No overdue credit cards');
      return results;
    }

    console.log(`📊 Found ${overdueCards.length} overdue card(s)`);
    ...
    console.log('✅ Credit card payment processing complete');
    ...
    console.error('❌ Error processing overdue credit cards:', error);
    throw error;
  }
}
...
export async function createCreditCardPayment(paymentData, card, paymentSource) {
  try {
    ...
    await logActivity(
      ...
      card.id,
      card.name,
      `Credit card payment: ${formatCurrency(amount)} for ${card.name} from ${paymentData.paymentSourceName} - Balance ${formatCurrency(previousCardBalance)} → ${formatCurrency(newCardBalance)}`,
      {
    ...
    console.log(`✅ Credit card payment created: ${card.name} - ${formatCurrency(amount)}`);
