...
export async function processOverdueLoanPayments(loans, creditCards = [], bankAccounts = [], cashInHand = 0) {
  try {
    ...
    console.log('🔄 Processing overdue loan payments...');
    ...
    console.log('✅ Loan payment processing complete');
...
export async function createLoanPayment(paymentData, loan, paymentSource) {
  try {
    ...
    await logActivity(
      ...
      loan.id,
      loan.name,
      `Loan payment: ${formatCurrency(amount)} for ${loan.name} from ${paymentData.paymentSourceName} - Balance ${formatCurrency(previousLoanBalance)} → ${formatCurrency(newLoanBalance)}`,
      {
    ...
    console.log(`✅ Loan payment created: ${loan.name} - ${formatCurrency(amount)}`);
